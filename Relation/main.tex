...
\puttitle{
	...
	name1=Matteo Vitali, % Author Name and Surname
...
Data span 11 metropolitan areas, including cities like Montreal, Toronto, Phoenix, Las Vegas, and Pittsburgh.
...
%AGGIUNGERE SE NECESSARIO LLA PARTE DI MODEL DEFINITION, OPPURE DIRE "RIMANDO AL NOTEBOOK PER LA SPEGAZIONE DEL CODICE"
\section{Objective}
%aggiungere che non è stat la prima cosa che abbiamo fatto, ma prima abbiamo fatto le queries (come gia detto), poi abbiamo aggiunto questa parte per studiare meglio la rlazione tra testo dele reviews e il resto.
\section{Methodology}
...
\chapter{Queries}
%Aggiungere nella query con lookup "notare quanto utile data normailization è stat per non dovre stare a controllare se un certo indice esiste pure nell'altra tabella"
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth]{imgs/businesses_and_reviews_per_category.png}
    \caption{Number businesses and reviews per category}
    \label{fig:num_businesses_and_reviews_per_category}
\end{figure}
The result shows that there are $1,311$ there is a wide variety of categories and businesses. \textit{"Restaurants"} and \textit{"Food"} categories dominate in terms of both the number of reviews ($4.7\text{M}$ and $1.8\text{M}$, respectively) and businesses ($52,268$ and $27,781$), making them highly competitive sectors. 
...
We say that a business is \textit{"polarizing"} if it is a one with high variance in star rating. The objective of this query is to identify, for each category, the most polarizing business.
...
As done in the previous query, we proceeded with a two-steps query. In the first one, all relevant documents were shaped and retrieved:
\begin{itemize}
...
All possible competitors were found by collecting businesses' names for each \texttt{(city, category)} in \texttt{businesses\_names}, duplicating this field, unwinding it, recollecting the businesses' names for each \texttt{(name, city)} into \texttt{possible\_competitors} and, finally, filtering it to exclude businesses with \texttt{name} equal to \texttt{\_id.name}. 
...
Then, using Pandas, widespread categories (i.e. categories in resulting DataFrame present in more than $15$ lines) were computed and for the top $3$ an histogram was plotted: 
